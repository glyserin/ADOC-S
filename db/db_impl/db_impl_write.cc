...
Status DBImpl::Put(const WriteOptions& o, ColumnFamilyHandle* column_family,
  ...
  uint64_t start_micros = immutable_db_options_.clock->NowMicros();
  ...
  Status result = DB::Put(o, column_family, key, val);
  
  uint64_t end_micros = immutable_db_options_.clock->NowMicros();
  uint64_t latency_micros = end_micros - start_micros;
  
  ROCKS_LOG_INFO(immutable_db_options_.info_log,
                 "[%s] PUT operation latency: %lu microseconds, key_size: %zu, value_size: %zu, status: %s",
                 column_family->GetName().c_str(), latency_micros, key.size(), val.size(),
                 result.ok() ? "OK" : result.ToString().c_str());
...
Status DBImpl::Put(const WriteOptions& o, ColumnFamilyHandle* column_family,
  ...
  Status result = DB::Put(o, column_family, key, ts, val);
  
  uint64_t end_micros = immutable_db_options_.clock->NowMicros();
  uint64_t latency_micros = end_micros - start_micros;
  
  ROCKS_LOG_INFO(immutable_db_options_.info_log,
                 "[%s] PUT (with timestamp) operation latency: %lu microseconds, key_size: %zu, value_size: %zu, status: %s",
                 column_family->GetName().c_str(), latency_micros, key.size(), val.size(),
                 result.ok() ? "OK" : result.ToString().c_str());
...
Status DBImpl::Merge(const WriteOptions& o, ColumnFamilyHandle* column_family,
  ...
  if (!cfh->cfd()->ioptions()->merge_operator) {
    result = Status::NotSupported("Provide a merge_operator when opening DB");
  } else {
    result = DB::Merge(o, column_family, key, val);
  }
  ...
  ROCKS_LOG_INFO(immutable_db_options_.info_log,
                 "[%s] MERGE operation latency: %lu microseconds, key_size: %zu, value_size: %zu, status: %s",
                 column_family->GetName().c_str(), latency_micros, key.size(), val.size(),
                 result.ok() ? "OK" : result.ToString().c_str());
...
Status DBImpl::Delete(const WriteOptions& write_options,
  ...
  Status result = DB::Delete(write_options, column_family, key);
  
  uint64_t end_micros = immutable_db_options_.clock->NowMicros();
  uint64_t latency_micros = end_micros - start_micros;
  
  ROCKS_LOG_INFO(immutable_db_options_.info_log,
                 "[%s] DELETE operation latency: %lu microseconds, key_size: %zu, status: %s",
                 column_family->GetName().c_str(), latency_micros, key.size(),
                 result.ok() ? "OK" : result.ToString().c_str());
...
Status DBImpl::Write(const WriteOptions& write_options, WriteBatch* my_batch) {
  ...
  ROCKS_LOG_INFO(immutable_db_options_.info_log,
                 "[BATCH] WRITE operation latency: %lu microseconds, batch_count: %d, batch_size: %zu, status: %s",
                 latency_micros, my_batch->Count(), my_batch->GetDataSize(),
...
Status DBImpl::WriteImpl(const WriteOptions& write_options,
  ...
  // Log write queue waiting time if the writer had to wait
  if (w.wait_time_micros > 0) {
    ROCKS_LOG_INFO(immutable_db_options_.info_log,
                   "[WRITE_QUEUE] Writer wait time: %lu microseconds, batch_size: %zu, final_state: %d",
                   w.wait_time_micros, WriteBatchInternal::ByteSize(w.batch), w.state.load());
  }
...
Status DBImpl::PipelinedWriteImpl(const WriteOptions& write_options,
  ...
  if (w.wait_time_micros > 0) {
    ROCKS_LOG_INFO(immutable_db_options_.info_log,
                   "[WRITE_QUEUE] Pipelined writer wait time: %lu microseconds, batch_size: %zu, final_state: %d",
                   w.wait_time_micros, WriteBatchInternal::ByteSize(w.batch), w.state.load());
  }
...
Status DBImpl::DelayWrite(uint64_t num_bytes,
  ...
  bool delayed = false;
  uint64_t stall_start_time = immutable_db_options_.clock->NowMicros();
  ...
  // Log write stall completion
  uint64_t stall_end_time = immutable_db_options_.clock->NowMicros();
  uint64_t total_stall_time = stall_end_time - stall_start_time;
  
  if (delayed || total_stall_time > 1000) { // Log if delayed or took more than 1ms
    ROCKS_LOG_INFO(immutable_db_options_.info_log,
                   "[WRITE_STALL] Write stall completed: total_time: %lu microseconds, "
                   "measured_delay: %lu microseconds, bytes: %lu, delayed: %s",
                   total_stall_time, time_delayed, num_bytes, 
                   delayed ? "true" : "false");
  }
