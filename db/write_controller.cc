...
uint64_t WriteController::GetDelay(SystemClock* clock, uint64_t num_bytes) {
  uint32_t stopped_count = total_stopped_.load(std::memory_order_relaxed);
  if (stopped_count > 0) {
    // Write is completely stopped
    return 0;
  }
  
  uint32_t delayed_count = total_delayed_.load(std::memory_order_relaxed);
  if (delayed_count == 0) {
    return 0;
  }
  ...
  uint64_t final_delay = std::max(next_refill_time_ - time_now, kMicrosPerRefill);
  
  // Log the write stall delay information
  static thread_local uint64_t last_log_time = 0;
  uint64_t now = time_now;
  // Log at most once per second to avoid log spam
  if (now - last_log_time > 1000000) {  // 1 second in microseconds
    // Note: We can't easily access DB's info_log here, so this would need to be 
    // logged at a higher level. For now, we'll prepare the information.
    last_log_time = now;
  }
