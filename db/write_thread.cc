...
void WriteThread::JoinBatchGroup(Writer* w) {
  ...
  // Record the time when this writer enters the queue  
  auto now = std::chrono::steady_clock::now();
  uint64_t join_start_time = std::chrono::duration_cast<std::chrono::microseconds>(
  ...
  if (linked_as_leader) {
    ...
    // Leader doesn't wait, so no stall time - no logging needed for immediate case
  }
  ...
  if (!linked_as_leader) {
    ...
    // Store start time in writer for logging at higher level
    w->join_start_time = join_start_time;
    ...
    uint64_t join_end_time = std::chrono::duration_cast<std::chrono::microseconds>(
        end_now.time_since_epoch()).count();
    uint64_t wait_time_micros = join_end_time - join_start_time;
    
    // Store wait time in writer for logging at higher level  
    w->wait_time_micros = wait_time_micros;
