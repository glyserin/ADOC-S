...
Status CompactionJob::Run() {
  ...
  // Log compaction start with level information
  Compaction* compaction = compact_->compaction;
  ColumnFamilyData* cfd = compaction->column_family_data();
  uint64_t start_time_micros = db_options_.clock->NowMicros();
  ...
  // Log compaction completion with level information and duration
  uint64_t end_time_micros = db_options_.clock->NowMicros();
  uint64_t duration_micros = end_time_micros - start_time_micros;
